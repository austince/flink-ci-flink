...
from pyflink.fn_execution.window_aggregate import SimpleNamespaceAggsHandleFunction, \
    ...
    def process_element_or_timer(self, input_datas: List[Tuple[int, Row, int, Row, int, int]]):
        ...
        for input_data in input_datas:
            if input_data[0] == NORMAL_RECORD:
                ...
                key = timer_data.key._values
                timer_type = timer_data.timer_type
                if self._is_time_window:
                    namespace = TimeWindow(timer_data.start, timer_data.end)
                else:
                    namespace = CountWindow(timer_data.id)
