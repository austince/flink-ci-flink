...
public class DemultiplexingRecordDeserializerTest {
    ...
    /** Tests {@link VirtualChannelSelector} by mixing buffers from 4 different virtual channels. */
    @Test
    public void testUpscale() throws IOException {
        DemultiplexingRecordDeserializer<Long> deserializer =
                DemultiplexingRecordDeserializer.create(
                        new InputChannelInfo(23, 0),
                        new InflightDataRescalingDescriptor(
                                set(0, 1),
                                singletonMap(23, mapping(set(2, 3), set(4, 5))),
                                emptySet()),
                        unused ->
        ...
        assertEquals(
                Sets.newSet(
                        new VirtualChannelSelector(0, 2),
                        new VirtualChannelSelector(0, 3),
                        new VirtualChannelSelector(1, 2),
                        new VirtualChannelSelector(1, 3)),
    ...
    public void testAmbiguousChannels() throws IOException {
        ...
        assertEquals(
                Sets.newSet(
                        new VirtualChannelSelector(41, 2),
                        new VirtualChannelSelector(41, 3),
                        new VirtualChannelSelector(42, 2),
                        new VirtualChannelSelector(42, 3)),
                deserializer.getVirtualChannelSelectors());
    ...
    public void testWatermarks() throws IOException {
        DemultiplexingRecordDeserializer<Long> deserializer =
                DemultiplexingRecordDeserializer.create(
                        new InputChannelInfo(37, 0),
                        new InflightDataRescalingDescriptor(
                                set(0, 1),
                                singletonMap(37, mapping(set(0, 1), set(4, 5))),
                                emptySet()),
                        unused ->
                                new SpillingAdaptiveSpanningRecordDeserializer<>(
                                        ioManager.getSpillingDirectoriesPaths()),
        ...
        for (Iterator<VirtualChannelSelector> iterator =
                        deserializer.getVirtualChannelSelectors().iterator();
                iterator.hasNext(); ) {
            VirtualChannelSelector selector = iterator.next();
            final BufferBuilder bufferBuilder = createBufferBuilder(128);
            final long ts =
