...
public class EventSerializer {
	...
	public static ByteBuffer toSerializedEvent(AbstractEvent event) throws IOException {
		...
		else if (eventClass == VirtualChannelSelector.class) {
			VirtualChannelSelector selector = (VirtualChannelSelector) event;
			ByteBuffer buf = ByteBuffer.allocate(12);
			buf.putInt(VIRTUAL_CHANNEL_SELECTOR_EVENT);
			buf.putInt(selector.getSubtaskIndex());
			buf.putInt(selector.getChannelIndex());
			buf.flip();
			return buf;
		}
		else {
	...
	public static AbstractEvent fromSerializedEvent(ByteBuffer buffer, ClassLoader classLoader) throws IOException {
		...
		try {
			...
			if (type == END_OF_PARTITION_EVENT) {
				...
				return new VirtualChannelSelector(buffer.getInt(), buffer.getInt());
			}
