    ...
    static NettyShuffleEnvironment createNettyShuffleEnvironment(
        ...
        BatchReadBufferPool ioBufferPool =
                new BatchReadBufferPool(
                        config.batchReadMemorySize().getBytes(), config.networkBufferSize());

        // at least 4 threads and at most 4 * (num cpu cores) threads
        int numThreads = ioBufferPool.getNumTotalBuffers() / ioBufferPool.getNumBuffersToRequest();
        ExecutorService ioExecutor =
                Executors.newFixedThreadPool(
                        Math.max(4, Math.min(numThreads, 4 * Hardware.getNumberCPUCores())),
