...
public class SortMergeResultPartitionReader implements Runnable, BufferRecycler {
    ...
    /**
     * Maximum time to wait in millisecond when requesting read buffers from the buffer pool before
     * throwing an exception (5min).
     */
    private static final long BUFFER_REQUEST_TIMEOUT = 5 * 60 * 1000;
    ...
    private final BatchReadBufferPool bufferPool;
    ...
    public SortMergeResultPartitionReader(
            BatchReadBufferPool bufferPool, Executor ioExecutor, Object lock) {
    ...
    public synchronized void run() {
        ...
        try {
            buffers.addAll(bufferPool.requestBuffers(this, BUFFER_REQUEST_TIMEOUT));
        } catch (Throwable throwable) {
            // this can happen only when the buffer pool is destroyed
            // or buffer request timeout so just release all readers
