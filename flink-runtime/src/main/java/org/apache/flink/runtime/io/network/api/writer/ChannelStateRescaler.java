...
public enum ChannelStateRescaler {
	...
	BROADCAST {
		@Override
		public Set<Integer> getOldChannels(int newChannelIndex, int oldNumberOfChannels, int newNumberOfChannels) {
			return IntStream.range(0, oldNumberOfChannels).boxed().collect(Collectors.toSet());
		}
	},
	...
	DISCARD_EXTRA_STATE {
		@Override
		public Set<Integer> getOldChannels(int newChannelIndex, int oldNumberOfChannels, int newNumberOfChannels) {
			return newChannelIndex >= oldNumberOfChannels ? emptySet() : singleton(newChannelIndex);
		}
	},
	...
	ARBITRARY {
		...
		public Set<Integer> getOldChannels(int newChannelIndex, int oldNumberOfChannels, int newNumberOfChannels) {
			...
			return ROUND_ROBIN.getOldChannels(newChannelIndex, oldNumberOfChannels, newNumberOfChannels);
		}
	},
	...
	FIRST_CHANNEL {
		@Override
		public Set<Integer> getOldChannels(int newChannelIndex, int oldNumberOfChannels, int newNumberOfChannels) {
			return newChannelIndex == 0 ?
				IntStream.range(0, oldNumberOfChannels).boxed().collect(Collectors.toSet()) :
				emptySet();
		}
	},
	...
	ROUND_ROBIN {
		...
		public Set<Integer> getOldChannels(int newChannelIndex, int oldNumberOfChannels, int newNumberOfChannels) {
			final Set<Integer> channels =
				Sets.newHashSetWithExpectedSize(newNumberOfChannels / oldNumberOfChannels + 1);
	 ...
	 * rescale.
	 */
	public abstract Set<Integer> getOldChannels(int newChannelIndex, int oldNumberOfChannels, int newNumberOfChannels);
}
