...
import static org.apache.flink.runtime.io.network.api.writer.ChannelStateRescaler.RANGE;
import static org.apache.flink.runtime.io.network.api.writer.ChannelStateRescaler.ROUND_ROBIN;
...
public class StateAssignmentOperationTest extends TestLogger {
	...
	public void testChannelStateAssignmentDownscaling() throws JobException, JobExecutionException {
		...
		assertEquals(
			new VirtualChannelMapping(bitset(0, 2), singletonList(mapping(bitset(0, 1), bitset(1, 2)))),
			getAssignedState(vertices.get(operatorIds.get(0)), operatorIds.get(0), 0).getOutputChannelMapping());
		assertEquals(
			new VirtualChannelMapping(bitset(1), singletonList(mapping(bitset(0, 1), bitset(1, 2)))),
			getAssignedState(vertices.get(operatorIds.get(0)), operatorIds.get(0), 1).getOutputChannelMapping());

		assertEquals(
			new VirtualChannelMapping(bitset(0, 1), singletonList(mapping(bitset(0, 2), bitset(1)))),
			getAssignedState(vertices.get(operatorIds.get(1)), operatorIds.get(1), 0).getInputChannelMapping());
		assertEquals(
			new VirtualChannelMapping(bitset(1, 2), singletonList(mapping(bitset(0, 2), bitset(1)))),
			getAssignedState(vertices.get(operatorIds.get(1)), operatorIds.get(1), 1).getInputChannelMapping());
	}

	@Nonnull
	private PartitionMapping mapping(BitSet... channelMappings) {
		return new PartitionMapping(asList(channelMappings));
	}

	private BitSet bitset(int... indexes) {
	...
	private Map<OperatorID, ExecutionJobVertex> buildVertices(
			...
			int parallelism,
			ChannelStateRescaler downstreamRescaler,
			ChannelStateRescaler upstreamRescaler) throws JobException, JobExecutionException {
		...
		for (int index = 1; index < jobVertices.length; index++) {
			...
			jobEdge.setDownstreamChannelStateRescaler(downstreamRescaler);
			jobEdge.setUpstreamChannelStateRescaler(upstreamRescaler);
		}
