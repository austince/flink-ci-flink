...
import static org.apache.flink.runtime.jobmaster.slotpool.SlotPoolUtils.createSlotPoolBuilder;
...
public class SlotPoolInteractionsTest extends TestLogger {
	...
	public void testCancelSlotAllocationWithoutResourceManager() throws Exception {
		...
		try (TestingSlotPoolImpl pool = createAndSetUpSlotPool(false)) {
	...
	public void testSlotAllocationTimeout() throws Exception {
		...
		try (TestingSlotPoolImpl pool = createAndSetUpSlotPool(true)) {
	...
	public void testExtraSlotsAreKept() throws Exception {
		...
		try (TestingSlotPoolImpl pool = SlotPoolUtils.createSlotPoolBuilder(
	...
	private TestingSlotPoolImpl createAndSetUpSlotPool(boolean connectToResourceManager) throws Exception {
		SlotPoolBuilder slotPoolBuilder = createSlotPoolBuilder(testMainThreadExecutor.getMainThreadExecutor());
		return connectToResourceManager ?
			slotPoolBuilder.build() :
			slotPoolBuilder.setResourceManagerGateway(null).build();
	}
}
