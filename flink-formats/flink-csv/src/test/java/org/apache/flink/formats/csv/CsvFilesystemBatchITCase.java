...
public class CsvFilesystemBatchITCase {
	...
	public static class EnrichedCsvFilesystemBatchITCase extends BatchFileSystemITCaseBase {
		...
		public void testFilenamePathMetadata() throws Exception {
			...
			check("select x,y,a,b,`flink.fs.path` from withMetadataTable",
				Arrays.asList(
			...
			check("select x,y,a,b,`flink.fs.path` from withMetadataTable2",
				Arrays.asList(
					Row.of("x8,8,1,1,file:" + file1.getPath()),
					Row.of("x8,8,2,2,file:" + file2.getPath()),
					Row.of("x8,8,3,3,file:" + file1.getPath())));

			check("select x,y,a,b,`flink.fs.path` from withMetadataTable3",
				Arrays.asList(
					Row.of("x8,8,1,1,file:" + file1.getPath()),
					Row.of("x8,8,2,2,file:" + file2.getPath()),
					Row.of("x8,8,3,3,file:" + file1.getPath())));
			check("select x,y,a,b,`flink.fs.path` from withMetadataTable5",
				Arrays.asList(
					//TODO: should path have a leading file:/ or not?
					Row.of("x8,8,1,1,file:" + file1.getPath()),
					Row.of("x8,8,2,2,file:" + file2.getPath()),
		...
		public void testBasenameMetadata() throws Exception {
			...
			check("select x,y,a,b,`flink.fs.basename` from withMetadataTable",
				Arrays.asList(
					Row.of("x8,8,1,1,test_file1"),
					Row.of("x8,8,2,2,test_file2"),
					Row.of("x8,8,3,3,test_file1")));
			check("select x,y,a,b,`flink.fs.basename` from withMetadataTable2",
				Arrays.asList(
					Row.of("x8,8,1,1,test_file1"),
					Row.of("x8,8,2,2,test_file2"),
					Row.of("x8,8,3,3,test_file1")));
			check("select x,y,a,b,`flink.fs.basename` from withMetadataTable4",
				Arrays.asList(
					Row.of("x8,8,1,1,test_file1"),
					Row.of("x8,8,2,2,test_file2"),
					Row.of("x8,8,3,3,test_file1")));
			check("select x,y,a,b,`flink.fs.basename` from withMetadataTable5",
				Arrays.asList(
					Row.of("x8,8,1,1,test_file1"),
					Row.of("x8,8,2,2,test_file2"),
					Row.of("x8,8,3,3,test_file1")));
		...
		public void testMetadata() throws Exception {
			...
			//  x  y  a b
			// x1  2  3 4
			check("select x,`flink.fs.path`,a,`flink.fs.basename`,y from withMetadataTable", // we drop field b
				Arrays.asList(
					//TODO: should path have a leading file:/ or not?
			...
			check("select x,`flink.fs.path`,b,a,`flink.fs.basename`,y from withMetadataTable",
				Arrays.asList(
			...
			check("select x,`flink.fs.path`,b,a,bname,y from withMetadataTable6",
				Arrays.asList(
					Row.of("x1,file:" + file1.getAbsolutePath() + ",4,3,test_file1,2"),
					Row.of("x5,file:" + file2.getAbsolutePath() + ",8,7,test_file2,6"),
					Row.of("x9,file:" + file1.getAbsolutePath() + ",12,11,test_file1,10")));

			check("select x,`flink.fs.path`,b,a,bname,y from withMetadataTable7",
				Arrays.asList(
					Row.of("x1,file:" + file1.getAbsolutePath() + ",4,3,test_file1,2"),
					Row.of("x5,file:" + file2.getAbsolutePath() + ",8,7,test_file2,6"),
					Row.of("x9,file:" + file1.getAbsolutePath() + ",12,11,test_file1,10")));
